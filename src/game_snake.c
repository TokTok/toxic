...
/* How long to wait for an opponent's state packet before timing out an online game */
#define SNAKE_ONLINE_TIMEOUT 15

/* Set to true to have the host controlled by a naive AI bot */
#define USE_AI false

#define SNAKE_ONLINE_VERSION 0x01
...
static void snake_game_over(SnakeState *state, Snake *snake, size_t snake_length)
{
    ...
    Snake *head = snake;
    head->colour = SNAKE_DEAD_BODY_COLOUR;
...
static void snake_grow(Snake *snake, size_t *length)
{
    ...
    snake[index].colour = snake->body_colour;
...
static void snake_initialize_snake_head(const GameData *game, SnakeState *state, bool is_online, bool self_host)
{
    ...
    other_head->colour = SNAKE_HEAD_OTHER_COLOUR;
...
static bool snake_packet_invite_respond(const GameData *game, const SnakeState *state)
{
    ...
    return game_packet_send(game, data, length, GP_Data) == 0;
}
...
{
    if (length != SNAKE_PACKET_STATE_LENGTH - 1 || data == NULL) {
        return false;
    }
    ...
    return snake_apply_state(game, state, other_direction, &other_coords, &food_coords);
}

static bool snake_handle_invite_response(const GameData *game, SnakeState *state, const uint8_t *data, size_t length)
{
    if (length != SNAKE_PACKET_INVITE_RESPONSE_LENGTH - 1 || data == NULL) {
        return false;
    }
    ...
    if (!game_coordinates_in_bounds(game, food_coords.x, food_coords.y)) {
        fprintf(stderr, "invalid food coords on initialization (x %u, y %u)\n", food_coords.x, food_coords.y);
        return false;
    }
