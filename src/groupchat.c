...
{
    free_ptr_array((void **) groupchats[groupnum].name_list, groupchats[groupnum].num_peers);
...
static void groupchat_onGroupNameListChange(ToxWindow *self, Tox *m, uint32_t groupnum)
{
    ...
    if (chat->name_list) {
        free_ptr_array((void **) chat->name_list, chat->num_peers);
        chat->name_list = NULL;
        chat->num_peers = 0;
    }
    ...
    chat->name_list = (char **) malloc_ptr_array(num_peers, TOX_MAX_NAME_LENGTH + 1, sizeof(char *));
    ...
    if (key == '\t') {  /* TAB key: auto-completes peer name or command */
        ...
        if (ctx->len > 0) {
            ...
            if (ctx->line[0] != L'/' || wcscmp(ctx->line, L"/me") == 0) {
                diff = complete_line(self, (char **) groupchats[self->num].name_list, groupchats[self->num].num_peers);
                ...
                diff = complete_line(self, (char **) group_cmd_list, sizeof(group_cmd_list) / sizeof(char *));
            }
