...
/*
 * Frees all non-null members of a pointer array of size `M` and the array itself.
 */
void free_ptr_array(void **arr, size_t M)
{
    if (arr == NULL) {
        return;
    }
...
/*
 * Returns a new array of M pointers of size `ptr_size`, each pointer allocated N bytes.
 * Returns NULL on failure.
 *
 * The caller is responsible for freeing the array with `free_ptr_array`.
 */
void **malloc_ptr_array(size_t M, size_t N, size_t ptr_size)
{
    void **arr = malloc(M * ptr_size);
