...
static void catch_SIGSEGV(int sig) {
  ...
  if (!freopen("/dev/tty", "w", stderr)) { // make sure stderr is enabled since
                                           // we may have disabled it
...
void cb_toxcore_logger(Tox *m, TOX_LOG_LEVEL level, const char *file,
...
/* One-time queue for messages created during init. Do not use after program
 * init. */
static void queue_init_message(const char *msg, ...) {
  ...
  char **new_msgs =
      realloc(init_messages.msgs, sizeof(char *) * init_messages.num);
...
static void print_init_messages(ToxWindow *toxwin) {
  ...
  for (i = 0; i < init_messages.num; ++i) {
    line_info_add(toxwin, NULL, NULL, NULL, SYS_MSG, 0, 0,
                  init_messages.msgs[i]);
  }
}
...
static void load_groups(Tox *m) {
  ...
  for (i = 0; i < num_chats; ++i) {
    ...
    if (init_groupchat_win(m, groupnum, type, (const char *)title, length) ==
        -1) {
      tox_conference_delete(m, groupnum, NULL);
      continue;
    }
  ...
  if (ch[0] == 'y' || ch[0] == 'Y') {
    ...
    while (valid_password == false) {
      ...
      if (string_is_empty(passconfirm) &&
          (len < MIN_PASSWORD_LEN || len > MAX_PASSWORD_LEN)) {
...
int store_data(Tox *m, const char *path) {
  ...
  if (user_password.data_is_encrypted && !arg_opts.unencrypt_data) {
    ...
    tox_pass_encrypt((uint8_t *)data, data_len, (uint8_t *)user_password.pass,
                     user_password.len, (uint8_t *)enc_data, &err);
...
static void init_tox_options(struct Tox_Options *tox_opts) {
  ...
  if (tox_options_get_tcp_port(tox_opts)) {
    queue_init_message("TCP relaying enabled on port %d",
                       tox_options_get_tcp_port(tox_opts));
  }
  ...
  if (tox_options_get_proxy_type(tox_opts) != TOX_PROXY_TYPE_NONE) {
    ...
    const char *ps =
        tox_options_get_proxy_type(tox_opts) == TOX_PROXY_TYPE_SOCKS5 ? "SOCKS5"
                                                                      : "HTTP";

    char tmp[sizeof(arg_opts.proxy_address) + MAX_STR_SIZE];
    snprintf(tmp, sizeof(tmp), "Using %s proxy %s : %d", ps,
  ...
  if (!tox_options_get_udp_enabled(tox_opts)) {
    ...
    const char *msg = "WARNING: Using a proxy without disabling UDP may leak "
...
static Tox *load_tox(char *data_path, struct Tox_Options *tox_opts,
  ...
  if (fp != NULL) { /* Data file exists */
    ...
    if (arg_opts.unencrypt_data && is_encrypted) {
      ...
      queue_init_message("Warning: passed --unencrypt-data option with "
                         "unencrypted data file '%s'",
                         data_path);
    }
    ...
    if (is_encrypted) {
      ...
      while (true) {
        fflush(stdout); // Flush before prompts so the user sees the
                        // question/message
        ...
        if (pweval) {
          ...
          pwlen =
              password_prompt(user_password.pass, sizeof(user_password.pass));
        }
        ...
        tox_pass_decrypt((uint8_t *)data, len, (uint8_t *)user_password.pass,
                         pwlen, (uint8_t *)plain, &pwerr);
...
static Tox *load_toxic(char *data_path) {
  ...
  if (new_err == TOX_ERR_NEW_PORT_ALLOC &&
      tox_options_get_ipv6_enabled(tox_opts)) {
...
static void print_usage(void) {
  ...
  fprintf(stderr,
          "  -e, --encrypt-data       Encrypt an unencrypted data file\n");
  fprintf(stderr, "  -f, --file               Use specified data file\n");
  fprintf(stderr, "  -h, --help               Show this message and exit\n");
  fprintf(stderr,
          "  -l, --logging            Enable toxcore logging in stderr\n");
  fprintf(stderr, "  -n, --nodes              Use specified DHTnodes file\n");
  fprintf(stderr,
          "  -o, --noconnect          Do not connect to the DHT network\n");
  fprintf(
      stderr,
      "  -p, --SOCKS5-proxy       Use SOCKS5 proxy: Requires [IP] [port]\n");
  fprintf(stderr,
          "  -P, --HTTP-proxy         Use HTTP proxy: Requires [IP] [port]\n");
  fprintf(stderr,
          "  -r, --namelist           Use specified name lookup server list\n");
  fprintf(stderr, "  -t, --force-tcp          Force toxic to use a TCP "
                  "connection (use with proxies)\n");
  fprintf(stderr, "  -T, --tcp-server         Act as a TCP relay server: "
                  "Requires [port]\n");
  fprintf(stderr,
          "  -u, --unencrypt-data     Unencrypt an encrypted data file\n");
...
static void parse_args(int argc, char *argv[]) {
  ...
  while ((opt = getopt_long(argc, argv, opts_str, long_opts, &indexptr)) !=
         -1) {
    switch (opt) {
    case '4':
      arg_opts.use_ipv4 = 1;
    ...
    case 'c':
      snprintf(arg_opts.config_path, sizeof(arg_opts.config_path), "%s",
               optarg);
    ...
    case 'p':
      ...
      snprintf(arg_opts.proxy_address, sizeof(arg_opts.proxy_address), "%s",
               optarg);
    ...
    case 'r':
      snprintf(arg_opts.nameserver_path, sizeof(arg_opts.nameserver_path), "%s",
               optarg);
...
/* Looks for an old default profile data file and blocklist, and renames them to
 * the new default names.
 *
...
static int rename_old_profile(const char *user_config_dir) {
  char old_data_file[strlen(user_config_dir) + strlen(CONFIGDIR) +
                     strlen(OLD_DATA_NAME) + 1];
  snprintf(old_data_file, sizeof(old_data_file), "%s%s%s", user_config_dir,
           CONFIGDIR, OLD_DATA_NAME);
  ...
  char old_data_blocklist[strlen(user_config_dir) + strlen(CONFIGDIR) +
                          strlen(OLD_DATA_BLOCKLIST_NAME) + 1];
  snprintf(old_data_blocklist, sizeof(old_data_blocklist), "%s%s%s",
           user_config_dir, CONFIGDIR, OLD_DATA_BLOCKLIST_NAME);
...
static void init_default_data_files(void) {
  ...
  if (config_err == -1) {
    ...
    DATA_FILE = malloc(strlen(user_config_dir) + strlen(CONFIGDIR) +
                       strlen(DATANAME) + 1);
    BLOCK_FILE = malloc(strlen(user_config_dir) + strlen(CONFIGDIR) +
                        strlen(BLOCKNAME) + 1);
  ...
  /* For backwards compatibility with old toxic profile names. TODO: remove this
   * some day */
...
int main(int argc, char **argv) {
  ...
  if (arg_opts.encrypt_data && arg_opts.unencrypt_data) {
    ...
    queue_init_message("Warning: Using --unencrypt-data and --encrypt-data "
  ...
  if (!datafile_exists && !arg_opts.unencrypt_data) {
    first_time_encrypt("Creating new data file. Would you like to encrypt it? "
                       "Y/n (q to quit)");
  ...
  if (nameserver_ret == -1) {
    queue_init_message(
        "curl failed to initialize; name lookup service is disabled.");
  } else if (nameserver_ret == -2) {
    queue_init_message("Name lookup server list could not be found.");
  } else if (nameserver_ret == -3) {
    queue_init_message(
        "Name lookup server list does not contain any valid entries.");
  }
  ...
  /* set user avatar from config file. if no path is supplied tox_unset_avatar
   * is called */
  char avatarstr[PATH_MAX + 11];
  snprintf(avatarstr, sizeof(avatarstr), "/avatar %s",
           user_settings->avatar_path);
  ...
  while (true) {
    ...
    if (user_settings->autosave_freq > 0 &&
        timed_out(last_save, user_settings->autosave_freq)) {
      pthread_mutex_lock(&Winthread.lock);
