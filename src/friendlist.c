...
void kill_friendlist(ToxWindow *self, FriendsList *friends, BlockedList *blocked, Windows *windows, const Client_Config *c_config)
{
    if (friends == NULL) {
        return;
    }
...
static void friendlist_add_blocked(FriendsList *friends, BlockedList *blocked, const Client_Config *c_config, uint32_t fnum,
                                   uint32_t bnum)
{
    realloc_friends(friends, friends->max_idx + 1);
    clear_friendlist_index(friends, friends->max_idx);
